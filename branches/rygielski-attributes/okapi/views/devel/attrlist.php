...
class View
{
	public static function call()
	{
		...
		foreach ($dict as $internal_id => $langs)
		{
			...
			if ($internal2acodes[$internal_id])
				print " - ".implode(", ", $internal2acodes[$internal_id]);
			print "\n";
			foreach ($langkeys as $langkey)
				print "        $langkey: ".$langs[$langkey]."\n";
		}

		print "\nAttribute notices - three priorities: (!), (-) and ( ) - the last one may be ignored:\n\n";
		$attrdict = AttrHelper::get_attrdict();
		foreach ($dict as $internal_id => $langs)
		{
			if (!isset($internal2acodes[$internal_id]))
			{
				print "(!) Attribute ".$internal_id." is not mapped to any A-code.\n";
				continue;
			}
			$acodes = $internal2acodes[$internal_id];
			if (count($acodes) == 1)
			{
				$acode = $acodes[0];
				$attr = $attrdict[$acode];
