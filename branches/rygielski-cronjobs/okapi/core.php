...
class Okapi
{
	...
	/**
	 * Check if any pre-request (important, run before dispatching the request)
	 * cronjobs are scheduled to execute and execute them if needed. Reschedule
	 * for new executions.
	 */
	public static function execute_prerequest_cronjobs()
	{
		...
		if ($nearest_event + 0 <= time())
		{
			...
			$nearest_event = CronJobController::run_jobs('before-request');
			Okapi::set_var("cron_nearest_event", $nearest_event);
		}
	}
	
	/**
	 * Check if any post-resonse (not so important, run after sending HTTP response)
	 * cronjobs are scheduled to execute and execute them if needed. Reschedule
	 * for new executions.
	 */
	public static function execute_postrequest_cronjobs()
	{
		set_time_limit(0);
		$nearest_event = Okapi::get_var("cron_nearest_event");
		if ($nearest_event + 0 <= time())
		{
			require_once 'cronjobs.php';
			$nearest_event = CronJobController::run_jobs('after-response');
			Okapi::set_var("cron_nearest_event", $nearest_event);
		}
	}
