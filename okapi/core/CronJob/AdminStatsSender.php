    ...
    public function execute()
    {
        ...
        $weekly_stats = Db::select_row("
            ...
            where
                ...
                and s.period_start > date_add(now(), interval -7 day)
        ");
        echo "Hello! This is your weekly summary of OKAPI usage.\n\n";
        echo 'Apps active this week: '.$active_apps_count.' out of '.$apisrv_stats['apps_count'].".\n";
        echo 'Total of '.$weekly_stats['total_http_calls'].' requests were made ('.sprintf('%01.1f', $weekly_stats['total_http_runtime'])." seconds).\n\n";
        ...
        ');
        echo "== Consumers ==\n\n";
        echo "Consumer name                         Calls     Runtime\n";
        echo "----------------------------------- ------- -----------\n";
        foreach ($consumers as $row) {
            ...
            echo self::mb_str_pad($name, 35, ' ', STR_PAD_RIGHT);
            echo str_pad($row['http_calls'], 8, ' ', STR_PAD_LEFT);
            echo str_pad(sprintf('%01.2f', $row['http_runtime']), 11, ' ', STR_PAD_LEFT)."s\n";
        }
        echo "\n";
        ...
        ');
        echo "== Methods ==\n\n";
        echo "Service name                          Calls     Runtime      Avg\n";
        echo "----------------------------------- ------- ----------- --------\n";
        ...
        $oauth_users = Db::select_all('
            ...
            order by count(*) desc;
        ');
        echo "== Current OAuth usage by Consumers with at least 5 users ==\n\n";
        echo "Consumer name                         Users\n";
        echo "----------------------------------- -------\n";
        foreach ($oauth_users as $row) {
            ...
            echo self::mb_str_pad($name, 35, ' ', STR_PAD_RIGHT);
            echo str_pad($row['users'], 8, ' ', STR_PAD_LEFT)."\n";
        }
        echo "\n";

        echo "This report includes requests from external consumers and those made via\n";
        echo "Facade class (used by OC code). It does not include methods used by OKAPI\n";
        echo "internally (i.e. while running cronjobs). Runtimes do not include HTTP\n";
        echo "request handling overhead.\n";
