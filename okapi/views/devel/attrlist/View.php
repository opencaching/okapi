    ...
    public static function call()
    {
        ...
        foreach ($dict as $internal_id => $langs) {
            ...
            if (in_array('en', $langkeys)) {
                echo strtoupper($langs['en']);
            } else {
                echo '>>>> ENGLISH NAME UNSET! <<<<';
            }
            if (isset($internal2acode[$internal_id])) {
                echo ' - '.$internal2acode[$internal_id];
            } else {
                echo ' - >>>> MISSING A-CODE MAPPING <<<<';
            }
            echo "\n";
            foreach ($langkeys as $langkey) {
                echo "        $langkey: ".$langs[$langkey]."\n";
            }
        }

        echo "\nAttribute notices:\n\n";
        echo "There are three priorities: (!), (-) and ( )\n";
        echo "(the last one ( ) can be safely ignored)\n\n";

        $attrdict = AttrHelper::get_attrdict();
        foreach ($dict as $internal_id => $langs) {
            if (!isset($internal2acode[$internal_id])) {
                echo '(!) Attribute '.$internal_id." is not mapped to any A-code.\n";
                continue;
            }
            ...
            foreach ($langs as $lang => $value) {
                ...
                if (!isset($attr['names'][$lang])) {
                    echo "(-) Attribute $acode is missing a name in the '$lang' language.\n";
                    echo "    Local name: $value\n";
                    echo "    OKAPI name: >> none <<\n";
                    continue;
                }
                if ($attr['names'][$lang] !== $value) {
                    echo "( ) Attribute $acode has a different name in the '$lang' language\n";
                    echo "    Local name: $value\n";
                    echo '    OKAPI name: '.$attr['names'][$lang]."\n";
                }
            }
        }
